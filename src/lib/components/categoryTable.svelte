...
<div class="p-4 bg-base-300 rounded-lg shadow-md">
    <div class="flex justify-between items-center mb-4">
        ...
        <div class="flex gap-3">
            ...
            />
            <button class="btn btn-sm" on:click={handleDelete}> Create </button>
            <button class="btn btn-sm" on:click={handleEdit}> Edit </button>
            <button
            ...
            <tbody>
                {#each parentCategories as category (category.category_id)}
                    ...
                    <tr class="bg-base-200">
                        ...
                        <td class="font-medium min-w-8 max-w-14"
                            >{category.name}</td
                        >
                        <td class="text-sm"
                    ...
                    {#if expanded[category.category_id]}
                        {#each filteredChildCategoriesMap[category.category_id] as child}
                            <tr transition:slide>
                                ...
                                <td class="min-w-8 max-w-14 overflow-hidden"
                                    >{child.name}</td
                                ><!-- TODO: place some type of icon to show that this is a chukd -->
                                <td class="text-sm min-w-8 max-w-14"
                                    >{child.description
                                        .replace(/<[^>]*>/g, "")
                                        .replace(/(.{35})..+/, "$1…")}</td
                                >
