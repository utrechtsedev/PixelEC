...
exports.editCategoryImage = async (req, res) => {
  try {
    ...
    // Find the image
    const image = await models.CategoryImage.findOne({ 
    ...
    // Check if setting as primary - need to ensure only one primary image per category
    if (is_primary === true && (!image.is_primary || image.is_primary !== true)) {
      // If this image is being set as primary, find any other primary images in the same category
      const otherPrimaryImages = await models.CategoryImage.findAll({ 
        where: { 
          category_id: image.category_id, 
          is_primary: true,
          image_id: {
            [Op.ne]: image_id // exclude the current image
          }
        }
      });
      
      // If there are existing primary images, update them to not be primary
      if (otherPrimaryImages && otherPrimaryImages.length > 0) {
        for (const primaryImage of otherPrimaryImages) {
          await primaryImage.update({ is_primary: false });
        }
      }
    }
    ...
    const updatedImage = await models.CategoryImage.findOne({ 
      where: { image_id: req.body.image_id }
    });
